            ...
            $(document).ready(function () {
                $("#enviar").on('click', function () {
                    if ($('#senha').val() && $('#confirmar_senha').val()) {
                        if ($('#senha').val() != $('#confirmar_senha').val()) {
                            bootbox.alert("As senha sao Diferente");
                            return false;
                        }
                    }
                });
                ...
                <c:if test="${usuario != null}">
                    ...
                    <c:if test="${usuario == null}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="name" aria-describedby="name" placeholder="Nome" name="name" required value="<c:out value='${usuario.name}' />">
                        </div>
                        <div class="form-group">
                            <label for="email">Email</label>
                            <input type="email" class="form-control" id="email" aria-describedby="email" placeholder="Email" name="email" required value="<c:out value='${usuario.email}' />">
                        </div>
                        <div class="form-group">
                            <label for="setor">Setor</label>
                            <input type="text" class="form-control" id="setor" aria-describedby="setor" placeholder="setor" required name="setor" value="<c:out value='${usuario.setor}' />">
                        </div>
                        <div class="form-group">
                            <label for="senha">Senha</label>
                            <input type="password" class="form-control" id="senha" aria-describedby="senha" placeholder="senha" name="senha" required value="<c:out value='${usuario.senha}' />">
                        </div>
                        <c:if test="${usuario == null}">
                            <div class="form-group">
                                <label for="confirmar_senha">Senha</label>
                                <input type="password" class="form-control" id="confirmar_senha" aria-describedby="confirmar_senha" placeholder="Confirmar Senha" name="confirmar_senha" required  />
                            </div>
                        </c:if>

                        <button type="submit" id="enviar" class="btn btn-primary" style="border-radius: 30px;" value="Save" >Enviar</button>
                    </form>
            </div>
        </div>
